...
function cancel () {
  ...
  if(fm430.deviceInitialized) {
    console.log("cancel: device is initialized")
    resetCamera();
  }
  else {
    console.log("cancel: device is not initialized")
    fm430.init(
        // On correct initialization
        () => {
          console.log('cancel: Initialization complete');
          resetCamera();
        },
...
function resetCamera(callback){
  console.log('DEBUG: fm430: running resetCamer()');
  fm430.ScanMode(FM430DataTypes.SCAN_MODE.LEVEL_MODE);
  fm430.Illumination(FM430DataTypes.ILLUMINATION.OFF);
  fm430.Aiming(FM430DataTypes.AIMING.AIMING_OFF, callback, callback);
}

function setUpCamera(){
  console.log('DEBUG: fm430: running setUpCamera()');
  fm430.Illumination(FM430DataTypes.ILLUMINATION.FADE_UP);
  fm430.ScanMode(FM430DataTypes.SCAN_MODE.CONTINUOUS_MODE); // Pulse mode or sense mode
