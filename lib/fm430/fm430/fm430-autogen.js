...
const FM430Protocol = require('./fm430-protocol.js');
...
Object.keys(proto).forEach(function(key) {
  ...
  else if(fun['argType'] === FM430DataTypes.TRAMSMIT_CHECK_CHARACTER) {
    // Data type validator
    validation += "    if(!Object.values(FM430DataTypes.TRAMSMIT_CHECK_CHARACTER).includes(" + argName + ")) {\n";
    validation += "      return errorCallback('Error in " + fun['name'] + ": Incorrect value in " + argName + " (' + " + argName + " + ')');\n    }\n";
  }
  else if(fun['argType'] === FM430DataTypes.TRAMSMIT_PREAMBLE_CHARACTER) {
    // Data type validator
    validation += "    if(!Object.values(FM430DataTypes.TRAMSMIT_PREAMBLE_CHARACTER).includes(" + argName + ")) {\n";
    validation += "      return errorCallback('Error in " + fun['name'] + ": Incorrect value in " + argName + " (' + " + argName + " + ')');\n    }\n";
  }
  ...
  else if(fun['argType'] === FM430DataTypes.MSI_PLEASSY_CHECK_CHARACTER_VERIFICATION) {
    // Data type validator
    validation += "    if(!Object.values(FM430DataTypes.MSI_PLEASSY_CHECK_CHARACTER_VERIFICATION).includes(" + argName + ")) {\n";
    validation += "      return errorCallback('Error in " + fun['name'] + ": Incorrect value in " + argName + " (' + " + argName + " + ')');\n    }\n";
  }
...
`
      datas = '    let datas = new Uint8Array([...' + argName1 + 'Arr2, 0x46, ...' + argName2 + 'Arr2, 0x54]);\n'
    }
...
`
      datas = '    let datas = new Uint8Array([...' + argName1 + 'Arr, 0x43, ...' + argName2 + 'Arr, 0x44]);\n'
    }
    else if(key == "FEBSDT"){
      argName = "delay";
  ...
  if(argName != "") {
    if(datas == "") {
      if(datasIsBytes) {
        datas = '    let datas = ' + argName + ';\n';
      } else {
        datas = '    let datas = new Uint8Array(Buffer.from(' + argName + '.toString()));\n';
      }
    }
    quary += 
`    let fm430_package = this.buildFM430Package(FM430DataTypes.STORAGE_TYPE.PERMANENT, [[` + toAscii(tag) + `]], [[` + toAscii(subTag) + `]], [datas]);
