...
function parse_HID_POS_package (hid_package) {
    ...
    if(reportID !== PC.REPORT_ID_RECEIVE) {
        console.error("parse_HID_POS_package: HID parser error: Incorrect report ID received, got " + utils.translateToASCII(PC.REPORT_ID_RECEIVE) + " got " + utils.translateToASCII(hid_package[0]));
        return false;
    }
    const length_of_command = hid_package[PC.LENGTH_POS];

    if(hid_package.length < length_of_command + PC.LENGTH_POS) {
        console.error("parse_HID_POS_package: HID parser error: Inconsistent length, hid_package.length (" + utils.translateToASCII(PC.REPORT_ID_RECEIVE) + ") < length_of_command + LENGTH_POS (" + (length_of_command + PC.LENGTH_POS));
        return false;
    }

    const command = hid_package.slice(PC.FM430_PACKAGE_POS, length_of_command + PC.FM430_PACKAGE_POS);
    return new Uint8Array(command);
}

// Adds HID POS protocol data:
function construct_HID_POS_package (fm430_package) {
    return [PC.REPORT_ID_SEND, fm430_package.length, ...fm430_package];
}
