...
FM430.prototype.shutdown = function () {
  //console.log('fm430: Running shutdown()');
...
FM430.prototype.buildFM430Package = function (storageType, tags, subTags, data=null) {
  ...
  pkg = new Uint8Array([...pkg, PC.TAG_DELIMITER, PC.ETX]);
  return pkg;
};
...
FM430.prototype.buildFM430TriggerPackage = function (triggerOn) {
  ...
  return new Uint8Array([PC.SOH, TRIGGER_STATE, PC.EOT]);
};
...
// hid_package comes in as a Uint8Array
FM430.prototype.load_incoming_hid_package = function (hid_package) {
  //console.log("fm430: Running load_incoming_hid_package()");
  //console.log("load_incoming_hid_package: type(hid_package): " + utils.dataType(hid_package));
  ...
  switch (FM430Protocol.get_fm430_package_type(fm430_package)) {
    case FM430DataTypes.PACKAGE_TYPE.COMMAND_PACKAGE:

      returnedData = FM430Protocol.parse_fm430_command_package(fm430_package);

      // Handle queue, execute the last callback if there were no errors, otherwise execute the error callback
      //console.log("queue shifting");
      //console.log("queue before: ");
      //utils.printQueue(this.packageQueue);
      const item = this.packageQueue.shift();
    ...
    case FM430DataTypes.PACKAGE_TYPE.DATA_PACKAGE_QUEUE:
      returnedData = FM430Protocol.parse_fm430_data_package(fm430_package);
      if(returnedData === false) {

        // On error: send back error callback, empty buffer, unregister resultReceiver
        this.asciiDataBuffer = "";
        ...
        this.asciiDataBuffer = this.asciiDataBuffer + returnedData;    // Add data to the buffer, dont do anything more
        //console.log("load_incoming_hid_package: found data package part of a queue, asciiDataBuffer after adding: " + this.asciiDataBuffer);
      }
      break;

    case FM430DataTypes.PACKAGE_TYPE.DATA_PACKAGE_END:
      returnedData = FM430Protocol.parse_fm430_data_package(fm430_package, true);

      // If someone is registered to retrieve data packages:
      if(this.resultsReceiver_resultCallback != null) {
        if(returnedData === false) {
          this.resultsReceiver_errorCallback('no data');
          this.asciiDataBuffer = ""; // Reset buffer on error
        }
        else {
          this.asciiDataBuffer = this.asciiDataBuffer + returnedData
          //console.log("load_incoming_hid_package: found end of data package, returning asciiDataBuffer: " + this.asciiDataBuffer);
          this.resultsReceiver_resultCallback(this.asciiDataBuffer);
          this.asciiDataBuffer = ""; // Reset buffer on end
        }
        if(this.resultsReceiver_type === 'one_shot') {
          this.unRegisterResultReceiver();
        }
    ...
    default:
      console.error("fm430 load_incoming_hid_package: Invalid package type");
      this.resultsReceiver_errorCallback('incorrect package retrived');
  }
};
...
FM430.prototype.tryExecuteNext = function() {
  ...
  if(this.packageQueue.length > 0) {
    if(!this.deviceBusy) {
      //console.log("queue: (picking no 0) ");
      //utils.printQueue(this.packageQueue);
...
FM430.prototype.enqueue = function(action, fm430_package, result_callback, error_callback) {
  ...
  //console.log("enqueue: type(fm430_package): " + utils.dataType(fm430_package));
...
FM430.prototype.write_to_fm430 = function (fm430_package) {
  ...
  //console.log("write_to_fm430: type(fm430_package): " + utils.dataType(fm430_package));
  if(!this.deviceBusy) {
    this.deviceBusy = true;
    ...
    console.error("write_to_fm430: Error: Device busy this should not happen");
  }
};
