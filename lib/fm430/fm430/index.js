...
FM430.prototype.init = function (initCompleteCallback, initErrorCallback) {
  ...
  this.hidDevice.on("data", this.load_hid_package.bind(this));
...
FM430.prototype.buildFM430Package = function (storageType, tags, subTags, data=null) {
  ...
  let pkg = [FM430Protocol.INITIAL_SEND, ...FM430Protocol.PREFIX, storageType];
  let previousTag = [];
  for(let i=0;i<tags.length;i++){
    if(utils.arraysEquals(tags[i], previousTag)) {
      if(data===null) {
        pkg = [...pkg, FM430Protocol.SUBTAG_DELIMITER, ...subTags[i]];
      }
      else {
        if(Array.isArray(data[i])){
          pkg = [...pkg, FM430Protocol.SUBTAG_DELIMITER, ...subTags[i], ...data[i]];
        }
        else {
          pkg = [...pkg, FM430Protocol.SUBTAG_DELIMITER, ...subTags[i], data[i]];
        }
      ...
      if(data===null) {
        pkg = [...pkg, FM430Protocol.TAG_DELIMITER, ...tags[i], ...subTags[i]];
      }
      else {
        if(Array.isArray(data[i])){
          pkg = [...pkg, FM430Protocol.TAG_DELIMITER, ...tags[i], ...subTags[i], ...data[i]];
        }
        else {
          pkg = [...pkg, FM430Protocol.TAG_DELIMITER, ...tags[i], ...subTags[i], data[i]];
        }
  ...
  pkg = [...pkg, FM430Protocol.TAG_DELIMITER, FM430Protocol.ETX];
  return pkg;
};
...
FM430.prototype.buildFM430TriggerPackage = function (triggerOn) {
  ...
  return [FM430Protocol.SOH, TRIGGER_STATE, FM430Protocol.EOT];
};
