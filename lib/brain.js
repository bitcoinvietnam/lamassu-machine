...
const Brain = function (config) {
  ...
  if(config.mockCam) {
    console.log("DEBUG: Initializing mock scanner")
    this.scanner = require('./mocks/scanner')
  }
  else if (config.camera.deviceType == "fm430"){
    console.log("DEBUG: Initializing fm430 scanner")
    this.scanner = require('./fm430/scanner')
  }
  else {
    console.log("DEBUG: Initializing scanner")
...
function _executeCallbackAfterASufficientIdlePeriod (callback) {
  ...
  const exitTime = config.exitTime  // 3h
  const exitOnIdle = exitTime + config.idleTime   // idleTime: 10m

  console.log("idle times: exitTime:", exitTime, "config.idleTime:", config.idleTime, "exitOnIdle:", exitOnIdle, "this.config.checkIdle:",this.config.checkIdle)
...
Brain.prototype._initBillValidatorEvents = function _initBillValidatorEvents () {
  ...
  billValidator.on('error', function (err) {
    console.log("on bill validator error")
    self._billValidatorErr(err) })
  billValidator.on('disconnected', function () {
    console.log("on bill validator disconnected")
    self._billValidatorErr() })
...
Brain.prototype.timeoutToScannerCancel = function timeoutToScannerCancel (stateMachine) {
  ...
  this.scannerTimeout = setTimeout(() => {
    console.log("DEBUG: Canceling scanner on timeout, timeoutToScannerCancel()")
    this.scanner.cancel()
...
Brain.prototype._processReal = function _processReal (req) {
  console.log("_processReal(), req.button: ", req.button)
  ...
  switch (req.button) {
    ...
    case 'pairingScanCancel':
      console.log("DEBUG: Canceling scanner for pairing scan")
      this.scanner.cancel()
      break
    case 'pairingErrorOk':
      this._unpaired()
...
Brain.prototype._testMode = function _testMode () {
  ...
  this.billValidator.run(function () {
    console.log("_testMode(), after billValidator.run()")
    self.trader.run()
    self._idle()
  })
}
...
Brain.prototype._isPendingScreen = function _isPendingScreen () {
  console.log("_isPendingScreen(), returning state: goodbye")
...
Brain.prototype.initValidator = function initValidator () {
  ...
  const h = setInterval(() => {
    ...
    return this.billValidator.run(err => {
      if (err){
        console.log("initValidator found error")
        return this._billValidatorErr(err)
      }
...
Brain.prototype._idle = function _idle (locale) {
  ...
  console.log("DEBUG: Canceling scanner due to idling state, _idle()")
  this.scanner.cancel()
...
Brain.prototype.smsCompliance = function smsCompliance (opts = {}) {
  ...
  flow.on('fail', () => {
    ...
    console.log("smsCompliance(), flow.on('fail'")
    this._idle()
  })
...
Brain.prototype.smsFlowHandleReturnState = function smsFlowHandleReturnState () {
  ...
  if (!returnState) {
    ...
    console.log("smsFlowHandleReturnState(), no return state")
    return this._idle()
  }
  ...
  if (returnState === 'acceptingFirstBill' && !isCompliant) {
    console.log("smsFlowHandleReturnState(), returnState === 'acceptingFirstBill'")
    return this._idle()
  }
...
Brain.prototype._setLocale = function _setLocale (data) {
  ...
  this._screenTimeout(function () { self._idle();   console.log("_setLocale() running timeout")}, 30000)
}
...
Brain.prototype._start = function _start () {
  ...
  if (printPaperWallet) {
    if (this.tx.cryptoCode !== 'BTC') {
      ...
      console.log("_start(), printPaperWallet, not BTC")
      return this._idle()
    }
...
Brain.prototype._startPrintedWalletScan = function _startPrintedWalletScan () {
  ...
  this.screenTimeout = setTimeout(() => {
    ...
    console.log("DEBUG: Canceling scanner on timeout for printed wallet scan")
    this.scanner.cancel()
...
Brain.prototype.handleUnresponsiveCamera = function handleUnresponsiveCamera () {
  console.log("DEBUG: Checking if scanner is responsive")
  if (!this.scanner.isOpened()) return this._idle()

  this._transitionState('maintenance')

  let handle = setInterval(() => {
    console.log("DEBUG: Checking if scanner is responsive on interval")
...
Brain.prototype.handleFM430Camera = function handleFM430Camera () {
  console.log("handleFM430Camera()")
  return this._idle()
}
...
Brain.prototype._scanActionCancel = function _scanActionCancel (fsm) {
  this.clearTimeoutToScannerCancel()
  console.log("DEBUG: Canceling scanner due to _scanActionCancel()")
  this.scanner.cancel()
  fsm.dispatch('FAIL')
}

Brain.prototype._cancelIdScan = function _cancelIdScan () {
  console.log("_cancelIdScan()")
  this.clearTimeoutToScannerCancel()
  this.startDisabled = false
  this._bye({timeoutHandler: () => { this.handleFM430Camera() }, timeout: this.config.goodbyeTimeout})
  console.log("DEBUG: Canceling scanner due to _cancelIdScan()")
  this.scanner.cancel()
}
...
Brain.prototype._startAddressScan = function _startAddressScan () {
  ...
  this.scanner.scanMainQR(this.tx.cryptoCode, (err, address) => {
    ...
    if (err) this.emit('error', err)
    console.log("scanMainQR(), returning state: scanAddress, goodbye")
    const startState = _.includes(this.state, ['scanAddress', 'goodbye'])
    const freshState = this.tx.id === txId && startState

    if (!freshState) return
    if (!address) {
      console.log("_startAddressScan(), no address")
      return this._idle()
    }
  ...
  this.screenTimeout = setTimeout(() => {
    ...
    console.log("DEBUG: Canceling scanner due to timeout, _startAddressScan()")
    this.scanner.cancel()
  }, this.config.qrTimeout)
}

Brain.prototype._bye = function _bye (opts) {
  console.log("_bye(), setting timed state, goodbye, opts: ", opts)
...
Brain.prototype._cancelScan = function _cancelScan () {
  console.log("_cancelScan()")
  this.startDisabled = false
  // TODO new-admin
  this._bye({timeoutHandler: () => { this.handleFM430Camera() }, timeout: this.config.goodbyeTimeout})
  console.log("DEBUG: Canceling scanner due to timeout _cancelScan()")
  this.scanner.cancel()
}

Brain.prototype._cancelInsertBill = function _cancelInsertBill () {
  console.log("_cancelInsertBill(), goint to idle state, disabling bill validator")
  this._idle()
  this.disableBillValidator()
}
...
Brain.prototype._pollUpdate = function _pollUpdate (needsRefresh) {
  ...
  this.sendRates()
  if (needsRefresh) {
    console.log("_pollUpdate(), needsRefresh")
    this._idle()
  }
}
  ...
  if (!this.hasConnected && this.state !== 'connecting') {
    ...
    setTimeout(function () {
      ...
      if (self.state === 'connecting') {
        console.log("_forceNetworkDown(), self.state=connecting")
        self._idle()
      }
...
Brain.prototype._networkUp = function _networkUp () {
  ...
  if (isNonTx(this.state)) {
    console.log("_networkUp(), isNonTx(this.state)")
    return this._idle()
  }
}

Brain.prototype._timedState = function _timedState (state, opts) {
  console.log("_timedState()")
  const self = this
  opts = opts || {}
...
Brain.prototype._firstBill = function _firstBill () {
  ...
  console.log("_firstBill(), setting screentimeout to _idle() in ", this.config.billTimeout)
  this._screenTimeout(() => {this._idle(); console.log("_firstBill(), _screenTimeout")}, this.config.billTimeout)
}
...
Brain.prototype.disableBillValidator = function disableBillValidator () {
  console.log("disableBillValidator(), emitting billValidatorOff")
...
Brain.prototype._billRejected = function _billRejected () {
  ...
  console.log("_billRejected(), setting timeout to either idle of sendCoins in ", this.config.billTimeout)
...
Brain.prototype._sendCoinsError = function _sendCoinsError (err) {
  ...
  if (err.statusCode === INSUFFICIENT_FUNDS_CODE) {
    setTimeout(function () { self._idle(); console.log("_sendCoinsError(), INSUFFICIENT_FUNDS_CODE") }, self.config.insufficientFundsTimeout)
...
Brain.prototype._completed = function _completed () {
  ...
  console.log("_completed, _screentimeout until idle: ", this.config.goodbyeTimeout)
...
Brain.prototype._billValidatorErr = function _billValidatorErr (err) {
  ...
  if (this.tx && this.tx.bills.length > 0) {
    ...
    console.log("sending credit information to browser")
    return
  }

  if (this.powerDown) return
  console.log("going into maintenence mode")
  self._transitionState('maintenance')
  console.log("emitting error in 15 seconds")
...
Brain.prototype._chooseFiat = function _chooseFiat () {
  ...
  const interval = setInterval(function () {
    ...
    console.log("_chooseFiat(), setInterval")
    self._idle()
  }, 120000)
}

Brain.prototype._chooseFiatCancel = function _chooseFiatCancel () {
  console.log("_chooseFiatCancel()")
  this._idle()
}
...
Brain.prototype._physicalDispense = function _physicalDispense () {
  ...
  batches.then(it => this._batchesFinished(it))
    .catch(err => {
      console.log(err)
      console.log("_physicalDispense(), error batches")
      return this._idle()
    })
}
...
Brain.prototype.cancelTransaction = function cancelTransaction (previousState) {
  switch (previousState) {
    case 'deposit':
      ...
      console.log("cancelTransaction(), deposit")
      this._idle()
      break
    case 'security_code':
    case 'register_phone':
    ...
    default :
      this.trader.cancelDispense(this.tx)
      console.log("cancelTransaction(), default")
      this._idle()
  }
}
